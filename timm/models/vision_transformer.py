...
class Attention(nn.Module):
    ...
    def forward(self, x, label_embed=None):
        ...
        if label_embed is not None: # For TL-Align
            attn_softmax = attn.detach().clone()
            attn_softmax = torch.mean(attn_softmax, dim=1)
            label_aligned = (attn_softmax @ label_embed).float()
...
class Block(nn.Module):
    ...
    def forward(self, x, label_embed=None):
        ...
        if label_embed is not None: # For TL-Align
            label_embed0 = label_embed
            y, label_embed = self.attn(self.norm1(x),label_embed=label_embed)
            label_embed = (label_embed + label_embed0) / 2.
            x = x + self.drop_path(y)
            x = x + self.drop_path(self.mlp(self.norm2(x)))
            return x, label_embed
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, label_embed=None):
        ...
        if label_embed is not None: #For TL-Align during training
            for blk in self.blocks:
                x,label_embed = blk(x,label_embed)

            x = self.norm(x)

            if self.dist_token is None:
                return self.pre_logits(x[:, 0]), label_embed[:, 0]
            else:
                return x[:, 0], x[:, 1], label_embed[:, 0]
        else:
            for blk in self.blocks:
                x = blk(x)
            x = self.norm(x)
    ...
    #
    def forward(self, x, label_embed=None):
        x = self.forward_features(x, label_embed=label_embed)
        if label_embed is not None: #For TL-Align during training

            if self.head_dist is not None:
                x, x_dist, label_embed = self.head(x[0]), self.head_dist(x[1]), x[2]  # x must be a tuple
                if self.training and not torch.jit.is_scripting():
                    # during inference, return the average of both classifier predictions
                    return x, x_dist, label_embed
                else:
                    return (x + x_dist) / 2, label_embed
            else:
                x, label_embed = x[0], x[1]
                x = self.head(x)
                return x, label_embed
        else:
